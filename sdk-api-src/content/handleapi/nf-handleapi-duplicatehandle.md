...
Objects created with a default security descriptor will prevent lower-privileged processes from duplicating handles into or out of higher-privileged processes or processes running in a different security context. Explicitly granting **PROCESS_DUP_HANDLE** to a process in a different security context effectively grants it full control of your process, so should generally be avoided. For more information, see 
<a href="https://docs.microsoft.com/windows/desktop/ProcThread/process-security-and-access-rights">Process Security and Access Rights</a>.
...
Not all "handles" can be duplicated with <b>DuplicateHandle</b>. The following types of object handles can be duplicated:
